...
#my new adjust lr
def _get_lr1(cfg, step):
    lr = cfg.lr
    if cfg.type == 'Cosine':  # Cosine Anneal
        start_step = cfg.get('start_step', 1)
        eta_min = 0.0
        #epoch_total=cfg.steps
        lr = eta_min + (cfg.lr - eta_min) * (1 + math.cos((math.pi * (step - cfg.get('warmup_steps',0))) / (cfg.steps-cfg.get('warmup_steps',0))))/ 2
    elif cfg.type == 'MultiStep':  # MultiStep
        num_steps = np.sum(step > np.asarray(cfg.decay_steps))
        lr = lr * (cfg.decay_rate ** num_steps)
    else:
...
def adjust_learning_rate1(cfg, optimizer, step, batch_idx=0, num_batches=100):
    start_step = cfg.get('start_step', 1)
    if step < cfg.get('warmup_steps', 0) + start_step:
        warmup_to = cfg.lr
        p = 1.0*(step - start_step) / (cfg.get('warmup_steps',0)-1)
        lr = cfg.warmup_from + p * (warmup_to - cfg.warmup_from)
    else:
        lr = _get_lr1(cfg, step)
...
def adjust_lr_simsiam1(cfg, optimizer, step):
    init_lr = cfg.lr
    #lr = _get_lr(cfg, step)
    start_step = cfg.get('start_step', 1)
    if step < cfg.get('warmup_steps', 0) + start_step:
...
def adjust_tau1(epoch,val):
    final_tau=1.0
    base_tau=0.99
    return (
            final_tau
            - (final_tau - base_tau) * (math.cos(math.pi * epoch /1000) + 1) / 2
        )
